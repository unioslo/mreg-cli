...
def format_hinfo(hinfo: dict, padding: int = 14) -> None:
    ...
    if hinfo is None:
        return ""
    return "{1:<{0}}cpu={2} os={3}\n".format(
        padding, "Hinfo:", hinfo["cpu"], hinfo["os"]
    )
...
def format_host_name(name: str, padding: int = 14) -> str:
    """Format given name."""
    if name is None:
        return
    assert isinstance(name, str)
    return "{1:<{0}}{2}\n".format(padding, "Name:", name)


def format_contact(contact: str, padding: int = 14) -> str:
    """Format given contact."""
    if contact is None:
        return
    assert isinstance(contact, str)
    return "{1:<{0}}{2}\n".format(padding, "Contact:", contact)


def format_comment(comment: str, padding: int = 14) -> str:
    """Format given comment."""
    if comment is None:
        return
    assert isinstance(comment, str)
    return "{1:<{0}}{2}\n".format(padding, "Comment:", comment)
...
def format_ttl(ttl: int, padding: int = 14) -> str:
    """Format given ttl"""
    assert isinstance(ttl, int) or ttl is None
    return "{1:<{0}}{2}\n".format(padding, "TTL:", ttl or "(Default)")


def format_loc(loc: dict, padding: int = 14) -> str:
    """Format given loc"""
    if loc is None:
        return ""
    assert isinstance(loc, dict)
    return "{1:<{0}}{2}\n".format(padding, "Loc:", loc["loc"])


def format_cname(cname: str, host: str, padding: int = 14) -> None:
    """Format given cname"""
    return "{1:<{0}}{2} -> {3}\n".format(padding, "Cname:", cname, host)


def format_mx(mxs: dict, padding: int = 14) -> str:
    """Format all MXs"""
    output = ""
    if not mxs:
        return ""
    len_pri = len("Priority")
    output += "{1:<{0}}{2} {3}\n".format(padding, "MX:", "Priority", "Server")
...
def format_naptr(naptr: dict, host_name: str, padding: int = 14) -> str:
    """Format given txt"""
    assert isinstance(naptr, dict)
    assert isinstance(host_name, str)


def format_ptr(ip: str, host_name: str, padding: int = 14) -> str:
    """Format given txt"""
    assert isinstance(ip, str)
    assert isinstance(host_name, str)
    return "{1:<{0}}{2} -> {3}\n".format(padding, "PTR override:", ip, host_name)


def format_txt(txt: str, padding: int = 14) -> str:
    """Format given txt"""
    if txt is None:
        return ""
    assert isinstance(txt, str)
    return "{1:<{0}}{2}\n".format(padding, "TXT:", txt)


def _format_host_info(info):
    """Format host info"""
    output = ""
    # Format all host info
    output += format_host_name(info["name"])
    output += format_contact(info["contact"])
    if info["comment"]:
        output += format_comment(info["comment"])
    output += format_ipadresses(info["ipaddresses"])
    for ptr in info["ptr_overrides"]:
        output += format_ptr(ptr["ipaddress"], info["name"])
    output += format_ttl(info["ttl"])
    output += format_mx(info["mxs"])
    output += format_hinfo(info["hinfo"])
    if info["loc"]:
        output += format_loc(info["loc"])
    for cname in info["cnames"]:
        output += format_cname(cname["name"], info["name"])
    for txt in info["txts"]:
        output += format_txt(txt["txt"])
    output += _srv_show(host_id=info["id"])
    naptr_info, naptr_count = _naptr_format(info)
    output += naptr_info
    sshfp_info, sshfp_count = _sshfp_format(info)
    output += sshfp_info
    cli_info("printed host info for {}".format(info["name"]))
    return output


def _format_ip_info(ip):
    """Format all hosts which have a given IP. Also print out PTR override, if any."""
    output = ""
    path = "/api/v1/hosts/"
    params = {
    ...
    output += format_ipadresses(ipaddresses, names=True)
    ...
    output += format_ptr(ip, ptrhost)
    return output


def _format_info(args):
    """Format information about host. If <name> is an alias the cname hosts info
    is shown.
    """
    output = ""
    for name_or_ip in args.hosts:
        ...
        if is_valid_ip(name_or_ip):
            ...
            if any(cname["name"] == name for cname in info["cnames"]):
                output += f'{name} is a CNAME for {info["name"]}'
            output += _format_host_info(info)
...
def find(args):
    ...
    output += _print("Name", "Contact", "Comment")
    for i in ret:
        output += _print(i["name"], i["contact"], i["comment"])
...
def _naptr_format(info):
    ...
    return output, len(naptrs)


def naptr_show(args):
    """Show all NAPTR records for host."""
    info = host_info_by_name(args.name)
    output, num_naptrs = _naptr_format(info)
    if num_naptrs == 0:
        return f"No naptrs for {info['name']}"
    ...
    if host:
        ...
        for ptr in host["ptr_overrides"]:
            if args.ip == ptr["ipaddress"]:
                padding = len(args.ip)
                output += format_ptr(args.ip, host["name"], padding)
    else:
        return f"No PTR found for IP '{args.ip}'"
...
def _srv_show(srvs=None, host_id=None):
    ...
    def format_srv(srv: dict, hostname: str, padding: int = 14) -> None:
        """Format given srv"""
        return "SRV: {1:<{0}} {2:^6} {3:^6} {4:^6} {5}\n".format(
            padding,
            srv["name"],
            srv["priority"],
            srv["weight"],
...
def sshfp_show(args):
    ...
    # Get host info or raise exception
    info = host_info_by_name(args.name)
    output, num_sshfps = _sshfp_format(info)
    if num_sshfps == 0:
...
def txt_show(args):
    ...
    output = ""
    for txt in txts:
        output += format_txt(txt["txt"], padding=5)
