...
# helper methods
def format_network_unused(count: int, padding: int = 25) -> str:
...
def format_network_excluded_ranges(info: dict, padding: int = 25) -> str:
    output = ""
    if not info:
        return ""
    count = 0
    for i in info:
        ...
        if end_ip == start_ip:
            count += 1
    output += "{1:<{0}}{2} ipaddresses\n".format(padding, "Excluded ranges:", count)
    for i in info:
        output += "{1:<{0}}{2} -> {3}\n".format(padding, "", i["start_ip"], i["end_ip"])
...
def format_network_reserved(ip_range: str, reserved: int, padding: int = 25) -> None:
    ...
    output += "{1:<{0}}{2}\n".format(padding, "Reserved host addresses:", reserved)
    output += "{1:<{0}}{2}{3}\n".format(padding, "", network.network_address, " (net)")
...
def format_network(info: int, text: str, padding: int = 25) -> None:
    return "{1:<{0}}{2}\n".format(padding, text, info)
...
def info(args):
    ...
    for net in args.networks:
        ...
        output += format_network(network_info["network"], "Network:")
        output += format_network(network.netmask.exploded, "Netmask:")
        output += format_network(network_info["description"], "Description:")
        output += format_network(network_info["category"], "Category:")
        output += format_network(network_info["location"], "Location:")
        output += format_network(network_info["vlan"], "VLAN")
        output += format_network(
            network_info["dns_delegated"] if network_info["dns_delegated"] else False,
            "DNS delegated:",
        )
        output += format_network(
            network_info["frozen"] if network_info["frozen"] else False, "Frozen"
        )
        output += format_network_reserved(
            network_info["network"], network_info["reserved"]
        )
        output += format_network_excluded_ranges(network_info["excluded_ranges"])
        output += format_network(used, "Used addresses:")
        output += format_network_unused(unused)
...
def list_used_addresses(args):
    ...
    if not ips:
        # cli_warning?
        output += f"No used addresses on {ip_range}\n"
        return

    for ip in ips:
        if ip in ptr2host:
            output += "{1:<{0}}{2} (ptr override)\n".format(25, ip, ptr2host[ip])
        elif ip in ip2host:
            if len(ip2host[ip]) > 1:
